...
async function setupCollections() {
  ...
  // Account fields — an exec becomes a login when `username` + `passwordHash`
  // are set. `accountRole` is what grants Sec-Gen access (no shared PIN).
  await str  ("executives", "username",        64,  false, null);
  await str  ("executives", "passwordHash",    120, false, null);
  await str  ("executives", "accountRole",     16,  false, "member");
  await bool ("executives", "accountActive",   false, true);
  await str  ("executives", "inviteTokenHash", 80,  false, null);
  await dt   ("executives", "inviteSentAt");
  await dt   ("executives", "inviteExpiresAt");
  await dt   ("executives", "lastLoginAt");
  ...
  // Google Docs topic guide — either pasted in or generated by Apps Script.
  await str  ("topics", "guideUrl",       2_000, false, null);
  await str  ("topics", "guideDocId",     200,   false, null);
  await str  ("topics", "guideTitle",     300,   false, null);
  await dt   ("topics", "guideCreatedAt");
  // Team ranking: one vote per account. `voteCount` is denormalized for sorting.
  await str  ("topics", "voters",         64,    false, null, /* array */ true);
  await int  ("topics", "voteCount",      false, 0);
}
...
async function setupIndexes() {
  ...
  await waitForAttributes("executives", ["active", "sortOrder", "name", "username", "accountRole"]);
  await index("executives", "active_sort_name", DatabasesIndexType.Key, ["active", "sortOrder", "name"], ["DESC", "ASC", "ASC"]);
  // A Key index, not Unique: most roster entries have no account, so `username`
  // is null on many rows, and a unique constraint over repeated nulls/empties is
  // not portable enough to bet roster additions on. Uniqueness is enforced in
  // lib/accounts.ts → validateUsername, which is the only path that sets one.
  await index("executives", "username",    DatabasesIndexType.Key, ["username"]);
  await index("executives", "accountRole", DatabasesIndexType.Key, ["accountRole"]);
  ...
  await waitForAttributes("topics", ["status", "category", "createdAt", "meetingId", "voteCount"]);
  ...
  await index("topics", "voteCount",  DatabasesIndexType.Key, ["voteCount"]);
}
